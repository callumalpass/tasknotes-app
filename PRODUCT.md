...
6. Keep reminders dependable. Mdbase owns reminder delivery so it does not depend on TaskNotes staying open.
7. Keep capture history available. Scratchpad is a chronological mixed stream of editable Markdown notes and independent image cards. Its Add image panel accepts drag and drop, multi-file upload, clipboard paste, and the mobile camera picker through one provider-neutral image pipeline. Note titles are optional and explicitly editable; TaskNotes never derives one from outline content, and an untitled historical note shows only its date. Historical notes and image cards can be collapsed, with collection-scoped collapse preferences retained locally across reloads and new-note transitions. The current note is the final item in the same fixed-height scrolling feed. When it fits, the feed opens with that note vertically centered; long notes fall back to keeping the active capture area near the bottom. Intentional upward scrolling reveals history and loading older items preserves the visible position.
