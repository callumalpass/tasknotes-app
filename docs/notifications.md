...
The settings screen starts in an explicit **Checking** state. Only
`Capacitor.isNativePlatform() === false` may produce **Available in the mobile
app**. Do not use that label as an initial placeholder: an unresolved native
plugin or network promise would otherwise make a real APK look like the web
application.
...
Unit tests cover browser/native distinction, missing native configuration,
